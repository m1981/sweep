...
def parse_patch_fcrs(fcr_patch_string: str):
    ...
    drops = [
        int(drop.group(1).strip()) for drop in drop_pattern.finditer(fcr_patch_string)
    ]
...
def validate_change(
    ...
    try:
        ...
        for file_path in cloned_repo.get_file_list():
            if file_path.endswith(code_suggestion.file_path):
                logger.info(
                    f"Found similar file {code_suggestion.file_path} at {file_path}"
                )
            ...
            if code_suggestion.original_code:
                logger.warning(
                    f"Failed to get file contents for {code_suggestion.file_path} due to {e}"
                )
    ...
    if not original_code:
        ...
        all_previous_occurrences = [
            x[1] - x[0] if x else 0 for x in previous_fcr_occurrences
        ]
        if (
            all_previous_occurrences
            and max(all_previous_occurrences) > len(original_code.splitlines()) // 2
        ):
        ...
        if (
            ...
            threshold = 50
            best_match, current_best_score = find_best_match(
                original_code, file_contents, threshold=threshold, tokenized=True
            )
            ...
            for indent_count in range(0, max_indentation, 2):
                match_score = fuzz.ratio(
                    indent(original_code, indent_count), best_match
                )
            ...
            if best_score != 100:
                if not check_valid_parentheses(best_match):
                    extended_match = find_smallest_valid_superspan(
                        best_match, file_contents
                    )
                    if (
                        extended_match
                        and extended_match.count("\n") - best_match.count("\n") < 20
                    ):
            ...
            if ext.removeprefix(".") in [
                ...
                for parentheses in ["()", "{}", "[]"]:
                    ...
                    old_parentheses_diff = original_code.count(
                        left
                    ) - original_code.count(right)
                    ...
                    if old_parentheses_diff != new_parentheses_diff:
                        ...
                        if new_parentheses_diff == 0:
                            best_superspan = find_smallest_valid_superspan(
                        ...
                        if not best_superspan:
                            ...
                            return (
                                error_message
                                + "Make sure the number of opening and closing parentheses match in both <original_code> and <new_code>, otherwise the changes will cause a syntax error."
                            )
...
def organize_snippets(
    snippets: list[Snippet], fuse_distance: int = 600
) -> list[Snippet]:
    """
    ...
    for i, snippet in enumerate(snippets):
        ...
        current_snippets = sort_and_fuse_snippets(
            current_snippets, fuse_distance=fuse_distance
        )
...
def get_max_snippets(
    ...
    for i in range(START_INDEX, 0, -1):
        expanded_snippets = [
            snippet.expand(expand * 2) if snippet.type_name == "source" else snippet
            for snippet in snippets[:i]
        ]
        proposed_snippets = organize_snippets(expanded_snippets[:i])
        cost = sum(
            [len(snippet.get_snippet(False, False)) for snippet in proposed_snippets]
        )
        if cost <= budget:
...
def format_snippets(
    ...
    joined_relevant_snippets = "\n".join(formatted_relevant_snippets)
...
def get_files_to_change(
    ...
    interleaved_snippets = partition_snippets_if_test(
        interleaved_snippets, include_tests=False
    )
    max_snippets = get_max_snippets(interleaved_snippets)
    relevant_snippets = [
        snippet
        for snippet in max_snippets
        if any(
            snippet.file_path == relevant_snippet.file_path
    ...
    if pr_diffs:
        messages.append(Message(role="user", content=pr_diffs, key="pr_diffs"))
    print("messages")
    ...
    if renames_dict:
        ...
        messages = [
            message
            if message.key != "relevant_snippets"
            else Message(
                role="user",
                content=format_snippets(
                    relevant_snippets,
        ...
        renames_string = (
            "# Warning\n<warnings>\nIMPORTANT:"
            + "\n".join(
                f"`{old_name}` has already been renamed to `{new_name}`"
                for old_name, new_name in renames_dict.items()
            )
            + "\nDo NOT include any renaming steps in your plan.\n</warnings>"
        )
        messages.append(Message(role="user", content=renames_string, key="renames"))
        joint_message = "\n\n".join(message.content for message in messages[1:])
        user_facing_message += (
    ...
    if not use_openai:
        ...
        issue_sub_request_pattern = re.compile(
            r"<issue_sub_requests>(.*?)</issue_sub_requests>", re.DOTALL
        )
        issue_sub_request_match = issue_sub_request_pattern.search(
            issue_sub_request_response
        )
        ...
        single_issue_sub_request_pattern = re.compile(
            r"<issue_sub_request>(.*?)</issue_sub_request>", re.DOTALL
        )
        for i, single_issue_sub_request_match in enumerate(
            single_issue_sub_request_pattern.finditer(issue_sub_requests)
        ):
            user_facing_message += (
                f"{i + 1}. {single_issue_sub_request_match.group(1).strip()}\n"
            )
        user_facing_message += "\n\n"
        yield renames_dict, user_facing_message, []
    ...
    issue_analysis_and_proposed_changes, failed, _ = extract_object_fields_from_string(
        proposed_plan_response, ["issue_analysis"]
    )
    ...
    if relevant_modules_match:
        relevant_modules = [
            relevant_module.strip()
            for relevant_module in relevant_modules_match.group(1).split("\n")
    ...
    try:
        for re_match in re.finditer(
            ...
            file_change_request.filename = renames_dict.get(
                file_change_request.filename, file_change_request.filename
            )
        ...
        yield (
            renames_dict,
            user_facing_message
            + "Here are the changes we decided to make. I'm currently just making some edits:\n",
            file_change_requests,
        )
        ...
        for error_resolution_count in range(3):
            ...
            chat_gpt.messages = [
                ...
                else Message(content=fix_files_to_change_system_prompt, role="system")
            ...
            fix_attempt = continuous_llm_calls(
                ...
                content=fix_files_to_change_prompt.format(
                    ...
                    allowed_indices=english_join(
                        [str(index) for index in range(len(error_indices))]
                    ),
                ),
                model=MODEL,
                temperature=0.1,
            ...
            for index, new_fcr in matches:
                ...
                if "COPIED_FROM_PREVIOUS_MODIFY" in new_fcr.instructions:
                    ...
                    file_change_requests[
                        error_indices[index]
                    ].filename = renames_dict.get(new_fcr.filename, new_fcr.filename)
                    continue
            ...
            error_message, error_indices = get_error_message(
                file_change_requests, cloned_repo, renames_dict=renames_dict
            )
...
def get_files_to_change_for_on_comment(
    ...
    messages.append(Message(role="user", content=formatted_pr_diffs, key="pr_diffs"))
    print("messages")
    for message in messages:
        print(message.content + "\n\n")
    joint_message = "\n\n".join(message.content for message in messages[1:])
...
def context_get_files_to_change(
    ...
    joined_relevant_snippets = "\n".join(
        relevant_snippet_template.format(
            ...
            if snippet.type_name == "source"
            else snippet.get_snippet(add_lines=False),
        )
    ...
    relevant_files_pattern = re.compile(
        r"<relevant_files>(.*?)</relevant_files>", re.DOTALL
    )
    ...
    read_only_files_pattern = re.compile(
        r"<read_only_files>(.*?)</read_only_files>", re.DOTALL
    )
...
def get_files_to_change_for_test(
    ...
    for relevant_snippet in relevant_snippets:
        if relevant_snippet.file_path in updated_files:
            relevant_snippet.content = updated_files[relevant_snippet.file_path][
                "contents"
            ]

    for read_only_snippet in read_only_snippets:
        if read_only_snippet.file_path in updated_files:
            read_only_snippet.content = updated_files[read_only_snippet.file_path][
                "contents"
            ]
    ...
    try:
        ...
        call_anthropic_second_time = (
            len(files_to_change_response) > max_tokens
            and files_to_change_response.count("</plan>") < expected_plan_count
        )
...
def get_files_to_change_for_gha(
    ...
    try:
        ...
        MODEL = "claude-sonnet-4-6" if not use_faster_model else "claude-sonnet-4-6"
        ...
        error_message, error_indices = get_error_message(
            file_change_requests, cloned_repo, updated_files
        )
        ...
        for _ in range(3):
            if not error_message:
                break
            chat_gpt.messages = [
                message for message in chat_gpt.messages if message.key != "system"
            ]
            fix_attempt = continuous_llm_calls(
                chat_gpt,
                content=fix_files_to_change_prompt.format(
            ...
            for index, new_fcr in matches:
                if index >= len(error_indices):
                    ...
                    continue
                if (
                    new_fcr.change_type == "create"
                    and "COPIED_FROM_PREVIOUS_CREATE" in new_fcr.instructions
                ):
                    # if COPIED_FROM_PREVIOUS_CREATE, we just need to override the filename
                    file_change_requests[
                        error_indices[index]
                    ].filename = new_fcr.filename
                    continue
                file_change_requests[error_indices[index]] = new_fcr
