...
def _safe_cache(path: str) -> Cache | None:
    ...
    for attempt in ("open", "rebuild"):
        try:
            cache = Cache(path)
            cache.check()          # raises if SQLite image is malformed
            return cache
...
# lexical_index_cache removed — tantivy persistence handled via cache_path
...
redis_client: Redis | None = (
    ...
    if (REDIS_URL and not FILE_CACHE_DISABLED)
    else None
)
...
def _token_cache_key(content: str) -> str:
    """
    SHA-256 of content + version string.
    Avoids storing full file contents as dict keys.
    """
    return hashlib.sha256(
        f"{content}{CACHE_VERSION}".encode()
    ).hexdigest()
...
class CustomIndex:
    ...
    _SEARCHER_POLL_INTERVAL = 0.01   # seconds
    _SEARCHER_MAX_POLLS     = 100

    def __init__(self, cache_path: str) -> None:
        os.makedirs(cache_path, exist_ok=True)
        # Pass cache_path so tantivy persists the index to disk
    ...
    def add_documents(self, documents: Iterable[tuple[str, str]]) -> None:
        ...
        def search_index(
                self, query: str
        ) -> list[tuple[str, float, tantivy.Document]]:
            """
    ...
    def _tokenize_with_cache(
        ...
        if workers > 1:
            ...
            missed_tokens = [
                tokenize_code(c)
                for c in tqdm(miss_contents, desc="Tokenizing documents")
            ]
