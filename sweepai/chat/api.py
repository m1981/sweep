...
def get_cloned_repo(
    repo_name: str, access_token: str, branch: str = None, messages: list[Message] = []
):
...
def posthog_trace(
    ...
    def wrapper(username: str, *args, metadata: dict = {}, **kwargs):
        ...
        if args:
            ...
            args_dict = dict(
                zip(args_names[1:], args)
            )  # skip first arg which must be username
        ...
        try:
            ...
            if (
                "metadata"
                in function.__code__.co_varnames[: function.__code__.co_argcount]
            ):
            ...
            posthog.capture(
                username, f"{function.__name__} success", properties=metadata
            )
            return result
...
def check_repo_exists_endpoint(
    repo_name: str, access_token: str = Depends(get_token_header)
):
...
def check_repo_exists(
    username: str, repo_name: str, access_token: str, metadata: dict = {}
):
...
def chat_codebase_stream(
    ...
    system_message = (
        ...
        else openai_system_message
    )
    chat_gpt: ChatGPT = ChatGPT.from_system_message_string(prompt_string=system_message)
    ...
    if pr_snippets:
        ...
        for snippet in snippets:
            if snippet.file_path in [
                pr_snippet.file_path for pr_snippet in pr_snippets
            ]:
            ...
            pr_files="\n".join(
                [
                    relevant_snippet_template.format(
                        i=i,
                        file_path=snippet.file_denotation,
                        content=snippet.expand(EXPAND_SIZE).get_snippet(
                            add_lines=False, add_ellipsis=False
                        ),
                    )
    ...
    chat_gpt.messages = [
        Message(content=chat_gpt.messages[0].content, role="system"),
        Message(content=snippets_message, role="user"),
        *messages[:-1],
    ]
    ...
    if len(messages) <= 2:
        chat_gpt.messages.append(
            Message(
                content=openai_format_message
                if use_openai
                else anthropic_format_message,
                role="user",
            )
        )
    ...
    def stream_state(
        ...
        for _ in range(5):
            ...
            for token in stream:
                ...
                if "<analysis>" in current_string:
                    analysis = (
                        extract_xml_tag(
                            current_string, "analysis", include_closing_tag=False
                        )
                        or ""
                    )
                    user_response = (
                        extract_xml_tag(
                            current_string, "user_response", include_closing_tag=False
                        )
                        or ""
                    )
                    self_critique = extract_xml_tag(
                        current_string, "self_critique", include_closing_tag=False
                    )
            ...
            if function_call:
                ...
                function_output, new_snippets = handle_function_call(
                    function_call, repo_name, fetched_snippets, access_token, k
                )
        ...
        code_suggestions_raw, _ = extract_objects_from_string(
            message_content, "code_change", ["file_path", "original_code", "new_code"]
        )
        ...
        for code_suggestion in code_suggestions_raw:
            fcr = next(
                (
                    ...
                    if fcr["file_path"] == code_suggestion["file_path"]
                    and fcr["original_code"] == code_suggestion["original_code"] == ""
                ),
                None,
            )
            if fcr:
        ...
        error_messages_dict = get_error_message_dict(
            file_change_requests=file_change_requests, cloned_repo=cloned_repo
        )
...
def handle_function_call(
    ...
    if function_call.function_name == "search_codebase":
        ...
        new_snippets_to_add = [
            ...
            if snippet.denotation not in fetched_snippet_denotations
        ]
        new_snippets_string = "\n".join(
            [
                relevant_snippet_template.format(
                    i=i, file_path=snippet.file_path, content=snippet.content
                )
    ...
    def stream():
        try:
            for stateful_code_suggestions in modify.stream(
                ...
                yield json.dumps(
                    [
                        stateful_code_suggestion.__dict__
                        for stateful_code_suggestion in stateful_code_suggestions
                    ]
                )
        except Exception as e:
...
async def commit_to_pull(
    ...
    logger.debug(
        f"Getting authenticated GitHub client took {timer.time_elapsed} seconds"
    )
    if not g:
        return {
            "success": False,
            "error": "The repository may not exist or you may not have access to this repository.",
        }
    ...
    cloned_repo = MockClonedRepo(
        f"{repo_cache}/{repo_name_}", repo_name, token=access_token, repo=repo
    )
...
async def validate_pull(
    ...
    def stream():
        try:
            ...
            any_failed = not all_statuses or any(
                status["succeeded"] is False for status in docker_statuses
            )
            if not any_failed:
                for _ in range(60 * 6):
                    ...
                    suite_runs = list(
                        repo.get_workflow_runs(
                            branch=pull_request.head.ref, head_sha=pull_request.head.sha
                        )
                    )
                    ...
                    if all(
                        ...
                        logger.info(
                            "All Github Actions have succeeded or have no result."
                        )
                        break
                    ...
                    for i, run in enumerate(
                        sorted(suite_runs, key=lambda run: run.name)
                    ):
    ...
    try:
        data = {
            ...
            "original_code_suggestions": [
                code_suggestion.__dict__
                if isinstance(code_suggestion, CodeSuggestion)
                else code_suggestion
                for code_suggestion in original_code_suggestions
            ],
            "code_suggestions": [
                code_suggestion.__dict__
                if isinstance(code_suggestion, CodeSuggestion)
